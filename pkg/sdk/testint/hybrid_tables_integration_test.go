...
func TestInt_HybridTables(t *testing.T) {
	...
	t.Run("alter operations", func(t *testing.T) {
		...
		t.Run("set comment", func(t *testing.T) {
			...
			// Overwrite comment (since UNSET is not supported)
			err = client.HybridTables.Alter(ctx, sdk.NewAlterHybridTableRequest(id).
				WithSet(*sdk.NewHybridTableSetPropertiesRequest().WithComment("")))
			require.NoError(t, err)
		...
		t.Run("add unique constraint", func(t *testing.T) {
			id, cleanup := testClientHelper().HybridTable.CreateWithColumns(t, []sdk.HybridTableColumnRequest{
				{Name: "ID", Type: sdk.DataType("NUMBER(38,0)"), InlineConstraint: &sdk.ColumnInlineConstraint{Type: sdk.ColumnConstraintTypePrimaryKey}},
				{Name: "EMAIL", Type: sdk.DataType("VARCHAR(200)")},
			})
			t.Cleanup(cleanup)

			constraint := *sdk.NewHybridTableOutOfLineConstraintRequest(sdk.ColumnConstraintTypeUnique).
				WithName("uq_email").WithColumns([]string{"EMAIL"})
			addAction := *sdk.NewHybridTableConstraintActionAddRequest(sdk.HybridTableOutOfLineConstraintRequest(constraint))
			err := client.HybridTables.Alter(ctx, sdk.NewAlterHybridTableRequest(id).
				WithConstraintAction(*sdk.NewHybridTableConstraintActionRequest().WithAdd(addAction)))
			require.NoError(t, err)
		})
